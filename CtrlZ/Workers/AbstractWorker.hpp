 ...
 * 类型是一切工人类型的基类，在这个类中指定了一些必须要实现的基本方法，这些方法将在调度器的调度流水线下被依次调用
 * 来实现工人的工作逻辑。通过将多个Worker类型的工人注册到调度器中，形成一个TaskList，用户可以实现复杂的工作流程，在构成一个流水线的同时，
 * 保证每个工人模块的逻辑是独立的，从而实现了工作流程的模块化和可扩展性。
 * @details
 * 这个类的主要目的是为了实现工人的工作逻辑，用户可以通过继承这个类来实现自己的工人类型，然后通过调度器来调度这些工人的工作流程。
 ...
 * 调度器类型，这个调度器类型会根据用户定义的调度逻辑依次调度工人的工作流程。
 */
...
class AbstractWorker : public ZObject {
 public:
   ...
   *
   * @param scheduler
   * 调度器的指针，可以通过调度器来获得一些全局的数据，比如当前任务的时间戳，前级工人的输出数据等
   * @param cfg 配置文件，用户可以通过配置文件来配置工人的一些参数。
   */
  AbstractWorker(SchedulerType::Ptr scheduler,
...
/**
 * @brief SimpleCallbackWorker
 * 类型是一个简单的回调工人类型，用户可以通过这个工人类型来实现一些简单的工作逻辑。
 * @details SimpleCallbackWorker
 * 类型是一个简单的回调工人类型，用户可以通过这个工人类型来实现一些简单的工作逻辑。
...
class SimpleCallbackWorker : public AbstractWorker<SchedulerType> {
  /// @brief
  /// 回调函数类型，函数签名为void(SchedulerType::Ptr)，包含一个调度器的指针
 ...
 public:
   ...
   * @param func
   * 回调函数，用户可以通过这个回调函数来实现工人的工作逻辑，该函数在TaskRun方法中被调用
   * @param cfg 配置文件，用户可以通过配置文件来配置工人的一些参数。
   */
  SimpleCallbackWorker(SchedulerType::Ptr scheduler, CallbackType func,
                       const nlohmann::json& cfg = nlohmann::json())
   ...
   *
   */
  virtual void TaskRun() override { callback__(this->Scheduler); }
