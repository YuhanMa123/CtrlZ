...
namespace z {
/**
 * @brief AMPInferenceWorker
 * 类型是一个人形机器人推理工人类型，该类实现了HumanoidGym网络兼容的推理功能。
 * @details AMPInferenceWorker
 * 类型是一个人形机器人推理工人类型，该类实现了HumanoidGym网络兼容的推理功能。
...
class AMPInferenceWorker
 ...
 public:
  /**
   * @brief 构造一个AMPInferenceWorker类型
   *
   * @param scheduler 调度器的指针
   * @param cfg 配置文件
   */
  AMPInferenceWorker(SchedulerType::Ptr scheduler,
    ...
    this->gait_air_ratio_l =
        NetworkCfg["Gait_air_ratio_l"].get<InferencePrecision>();
    this->gait_air_ratio_r =
        NetworkCfg["Gait_air_ratio_r"].get<InferencePrecision>();
    this->gait_phase_offset_l =
        NetworkCfg["Gait_phase_offset_l"].get<InferencePrecision>();
    this->gait_phase_offset_r =
        NetworkCfg["Gait_phase_offset_r"].get<InferencePrecision>();
    ...
    std::cout << "Gait_air_ratio_l=" << this->gait_air_ratio_l << std::endl;
    std::cout << "Gait_air_ratio_r=" << this->gait_air_ratio_r << std::endl;
    std::cout << "Gait_phase_offset_l=" << this->gait_phase_offset_l
              << std::endl;
    std::cout << "Gait_phase_offset_r=" << this->gait_phase_offset_r
              << std::endl;
    std::cout << "dt=" << this->dt << std::endl;
    this->PrintSplitLine();

    // concatenate all scales
    auto sin_clock_scales = math::Vector<InferencePrecision, 2>::ones();
  ...
  void PreProcess() override {
    ...
    // t = episode_length_buf * dt / gait_cycle
    size_t t = this->Scheduler->getTimeStamp();
    InferencePrecision t_norm =
        this->dt * static_cast<InferencePrecision>(t) / this->cycle_time;

    // std::cout << "dt: " << this->dt << std::endl;
    // std::cout << "t_norm: " << t_norm << std::endl;

    // gait_phase[:, 0] = (t + phase_offset[:, 0]) % 1.0
    // gait_phase[:, 1] = (t + phase_offset[:, 1]) % 1.0
    InferencePrecision phase_L =
        std::fmod(t_norm + this->gait_phase_offset_l,
                  static_cast<InferencePrecision>(1.0));
    InferencePrecision phase_R =
        std::fmod(t_norm + this->gait_phase_offset_r,
                  static_cast<InferencePrecision>(1.0));
    // 确保相位为正
    if (phase_L < 0) phase_L += static_cast<InferencePrecision>(1.0);
    if (phase_R < 0) phase_R += static_cast<InferencePrecision>(1.0);

    // clock: sin(2*pi*phase), cos(2*pi*phase) 分别对左右腿
    z::math::Vector<InferencePrecision, 2> GaitPhaseLR = {phase_L, phase_R};
    z::math::Vector<InferencePrecision, 2> ClockSin = {
        std::sin(static_cast<InferencePrecision>(2.0 * M_PI) * phase_L),
        std::sin(static_cast<InferencePrecision>(2.0 * M_PI) * phase_R)};
    z::math::Vector<InferencePrecision, 2> ClockCos = {
        std::cos(static_cast<InferencePrecision>(2.0 * M_PI) * phase_L),
        std::cos(static_cast<InferencePrecision>(2.0 * M_PI) * phase_R)};

    // phase_ratio: 定值，与训练配置一致 (gait_air_ratio_l, gait_air_ratio_r)
    z::math::Vector<InferencePrecision, 2> PhaseRatio = {gait_air_ratio_l,
                                                         gait_air_ratio_r};

    // #TODO: 这里就有意思了，发布的信息不对，但是好像没什么影响
 ...
 private:
  ...
  InferencePrecision gait_air_ratio_l;
  InferencePrecision gait_air_ratio_r;
  InferencePrecision gait_phase_offset_l;
  InferencePrecision gait_phase_offset_r;
