...
/**
 * @brief
 * AbstractNetInferenceWorker类型是一切神经网络推理工人类型的基类，该类提供一些基本推理的功能，
 ...
 * //输入节点名称 "OutputNodeNames": ["output"] //输出节点名称
 * 				}
 * 			}
 * 		}
 * }
...
class AbstractNetInferenceWorker : public AbstractWorker<SchedulerType> {
 ...
 public:
   ...
   * @param cfg 配置文件
   */
  AbstractNetInferenceWorker(SchedulerType::Ptr scheduler,
    ...
    this->IntraNumberThreads__ =
        InferenceCfg["IntraNumberThreads"].get<size_t>();

    // 读取网络配置
    this->ModelPath__ = NetworkCfg["ModelPath"].get<std::string>();
    this->InputNodeNames__ =
        NetworkCfg["InputNodeNames"].get<std::vector<std::string>>();
    this->OutputNodeNames__ =
        NetworkCfg["OutputNodeNames"].get<std::vector<std::string>>();

    // 初始化模型
    // this->SessionOptions__ = Ort::SessionOptions();
    this->SessionOptions__.SetGraphOptimizationLevel(
...
#endif
    this->Session__ =
        Ort::Session(GetOrtEnv(), wstr.c_str(), this->SessionOptions__);
    this->MemoryInfo__ =
        Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);

    this->PrintSplitLine();
    std::cout << "AbstractNetInferenceWorker" << std::endl;
    std::cout << "IntraNumberThreads=" << this->IntraNumberThreads__
              << std::endl;
    ...
    for (auto&& name : this->InputNodeNames__) std::cout << name << " ";
    std::cout << "]\n";

    std::cout << "OutputNodeNames=[";
    for (auto&& name : this->OutputNodeNames__) std::cout << name << " ";
    std::cout << "]\n";
   ...
   * @param enable 是否启用推理
   * @param future_time
   * 在未来的某个时间点启用推理，单位为秒，默认值为0，表示立即启用
   */
  void SetEnable(bool enable, InferencePrecision future_time = 0) {
    ...
    this->NextActionCycleCnt__.store(
        this->Scheduler->getTimeStamp() +
        static_cast<size_t>(future_time / this->Scheduler->getSpinOnceTime()));
  }
  ...
  void TaskCreate() override {
    ...
    if (this->InputOrtTensors__.empty() || this->OutputOrtTensors__.empty())
      throw(std::runtime_error(
          "InputOrtTensors or OutputOrtTensors is empty, call WarpOrtTensor to "
          "create Ort tensors before launch scheduler!"));

    if (this->InputNodeNames__.size() != this->InputOrtTensors__.size())
      throw(std::runtime_error(
          "InputNodeNames size is not equal to InputOrtTensors size!"));
    if (this->OutputNodeNames__.size() != this->OutputOrtTensors__.size())
      throw(std::runtime_error(
          "OutputNodeNames size is not equal to OutputOrtTensors size!"));

    this->IoBinding__ = Ort::IoBinding(this->Session__);

    for (size_t i = 0; i < this->InputNodeNames__.size(); i++) {
      this->IoBinding__.BindInput(this->InputNodeNames__[i].c_str(),
                                  this->InputOrtTensors__[i]);
    }

    for (size_t i = 0; i < this->OutputNodeNames__.size(); i++) {
      this->IoBinding__.BindOutput(this->OutputNodeNames__[i].c_str(),
                                   this->OutputOrtTensors__[i]);
    }
  ...
  Ort::Value WarpOrtTensor(math::Tensor<InferencePrecision, Dims...>& Tensor) {
    return Ort::Value::CreateTensor<InferencePrecision>(
        this->MemoryInfo__, Tensor.data(), Tensor.size(), Tensor.shape_ptr(),
        Tensor.num_dims());
  }
