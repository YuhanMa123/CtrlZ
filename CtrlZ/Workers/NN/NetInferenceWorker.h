...
math::Vector<Scalar, 3> ComputeProjectedGravity(
    math::Vector<Scalar, 3>& EularAngle,
    const math::Vector<Scalar, 3>& GravityVector = {0, 0, -1}) {
  static_assert(std::is_arithmetic<Scalar>::value,
                "Scalar must be a arithmetic type");
  math::Vector<Scalar, 4> quat = math::quat_from_euler_xyz(EularAngle);
  math::Vector<Scalar, 3> pravity_vec =
      math::quat_rotate_inverse(quat, GravityVector);
  return pravity_vec;
}
};  // namespace z
