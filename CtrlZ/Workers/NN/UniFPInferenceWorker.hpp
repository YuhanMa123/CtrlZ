...
#include "CommonLocoInferenceWorker.hpp"  //推理工作基类
#include "NetInferenceWorker.h"           //网络推理接口
#include "Utils/StaticStringUtils.hpp"    //字符串工具
#include "Utils/ZenBuffer.hpp"            //缓冲区
#ifndef M_PI
...
template <typename SchedulerType, CTString NetName, typename InferencePrecision,
          size_t INPUT_STUCK_LENGTH, size_t JOINT_NUMBER>  // 创建模板类

class UniFPInferenceWorker  // 模板类定义+继承
    : public CommonLocoInferenceWorker<SchedulerType, NetName,
                                       InferencePrecision, JOINT_NUMBER> {
 public:  // 有点像python的_init_buffer
 ...
 public:
  /**
   * @brief 构造一个UniFPInferenceWorker类型,构造构造函数
   *
   * @param scheduler 调度器的指针
   * @param cfg 配置文件
   */
  UniFPInferenceWorker(SchedulerType::Ptr scheduler,
    ...
    InferencePrecision scales_base_force =
        obs_scale_cfg["base_force"].get<InferencePrecision>();

    InferencePrecision lin_vel = this->Scales_command3[0];
    InferencePrecision ang_vel = this->Scales_command3[2];
    this->Scales_command6 = {lin_vel,           lin_vel,
                             ang_vel,           scales_base_force,
                             scales_base_force, scales_base_force};

    // 打印初始化信息到控制台，显示关键参数
    this->PrintSplitLine();
    std::cout << "UniFPInferenceWorker" << std::endl;
    std::cout << "JOINT_NUMBER=" << JOINT_NUMBER << std::endl;
    std::cout << "Cycle_time=" << this->cycle_time << std::endl;
    std::cout << "dt=" << this->dt << std::endl;
    std::cout << "Scales_command6=" << this->Scales_command6 << std::endl;
    this->PrintSplitLine();
  ...
  void PreProcess() override {
    this->start_time = std::chrono::steady_clock::now();  // 记录开始时间
    ...
    CurrentMotorPos -= this->JointDefaultPos;  // 减去默认位置得到相对位置
    ...
    // ###############################################################################
    // TODO:
    // 这部分的逻辑还需要再梳理，训练中当command小于阈值时，将sin置零，说实话那个sin的步态周期也很奇怪，这部分需要重新评估
    ...
    // walking mask: only when command exceeds small clips (same as training)
    bool walking = std::abs(UserCmd6(0)) > lin_vel_x_clip_ ||
                   std::abs(UserCmd6(1)) > lin_vel_y_clip_ ||
                   std::abs(UserCmd6(2)) > ang_vel_yaw_clip_;

    if (walking) {
      gait_index_ += this->dt / this->cycle_time;
      if (gait_index_ >= static_cast<InferencePrecision>(1.0)) {
        gait_index_ -= std::floor(gait_index_);
      }
    } else {
      gait_index_ = static_cast<InferencePrecision>(0.0);
    }

    InferencePrecision phase = gait_index_;
    InferencePrecision clock_sin =
        std::sin(phase * static_cast<InferencePrecision>(2.0 * M_PI));
    InferencePrecision clock_cos =
    ...
    auto SingleInputVecScaled =
        math::cat(ProjectedGravity, AngVel, CurrentMotorPos, CurrentMotorVel,
                  LastAction, ClockVector, UserCmd6) *
        this->InputScaleVec;
 ...
 private:
  ...
  static constexpr size_t INPUT_TENSOR_LENGTH_UNIT =
      3 + 3 + JOINT_NUMBER + JOINT_NUMBER + JOINT_NUMBER + 2 + 6;
  ...
  // latent dim
  static constexpr size_t LATENT_DIM = 6;

  // input tensor
  z::math::Tensor<InferencePrecision, 1, INPUT_TENSOR_LENGTH>
      InputTensor;  // InputTensor是张量，维度为（1，INPUT_TENSOR_LENGTH）
  ...
  // output tensor
  z::math::Tensor<InferencePrecision, 1, OUTPUT_TENSOR_LENGTH>
      OutputTensor;  // OutputTensor是张量，维度为（1，OUTPUT_TENSOR_LENGTH）
  ...
  // gait phase (only advances when walking command is non-zero)
  InferencePrecision gait_index_ = 0;

  // command thresholds for walking mask (match training cfg.commands.*_clip)
  InferencePrecision lin_vel_x_clip_ = static_cast<InferencePrecision>(0.1);
  InferencePrecision lin_vel_y_clip_ = static_cast<InferencePrecision>(0.1);
  InferencePrecision ang_vel_yaw_clip_ = static_cast<InferencePrecision>(0.2);

  ValVec6 Scales_command6;
};
};  // namespace z
