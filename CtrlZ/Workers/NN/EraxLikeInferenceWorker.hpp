...
template <typename SchedulerType, CTString NetName, typename InferencePrecision,
          size_t INPUT_STUCK_LENGTH, size_t EXTRA_INPUT_LENGTH,
          size_t JOINT_NUMBER>
class EraxLikeInferenceWorker
 ...
 public:
  EraxLikeInferenceWorker(SchedulerType::Ptr scheduler,
                          const nlohmann::json& Net_Config,
                          const nlohmann::json& Motor_Config)
      : CommonLocoInferenceWorker<SchedulerType, NetName, InferencePrecision,
                                  JOINT_NUMBER>(scheduler, Net_Config,
                                                Motor_Config),
    ...
    this->InputScaleVec = math::cat(
        this->Scales_ang_vel, this->Scales_project_gravity,
  ...
  void PreProcess() override {
    ...
    ValVec3 ProjectedGravity =
        ComputeProjectedGravity(Ang, this->GravityVector);
    this->Scheduler->template SetData<concat(NetName, "NetProjectedGravity")>(
        ProjectedGravity);

    size_t t = this->Scheduler->getTimeStamp();
    InferencePrecision clock = 2 * M_PI / this->CyctleTime * this->dt *
                               static_cast<InferencePrecision>(t);
    ...
    z::math::Vector<InferencePrecision, EXTRA_INPUT_LENGTH> ExtraInputVec =
        math::cat(UserCmd3, ClockVector);

    auto SingleInputVecScaled =
        math::cat(AngVel, ProjectedGravity, CurrentMotorPos, CurrentMotorVel,
                  LastAction) *
        this->InputScaleVec;
    ...
    constexpr size_t EXTRA_OFFSET =
        INPUT_STUCK_LENGTH * INPUT_TENSOR_LENGTH_UNIT;
    std::copy(ExtraInputVec.begin(), ExtraInputVec.end(),
              InputVec.begin() + EXTRA_OFFSET);
  ...
  void PostProcess() override {
    ...
    InferencePrecision inference_time =
        static_cast<InferencePrecision>(duration.count()) / 1000.0;
 ...
 private:
  ...
  static constexpr size_t INPUT_TENSOR_LENGTH_UNIT =
      3 + 3 + JOINT_NUMBER + JOINT_NUMBER + JOINT_NUMBER;
  static constexpr size_t INPUT_TENSOR_LENGTH =
      INPUT_TENSOR_LENGTH_UNIT * INPUT_STUCK_LENGTH + EXTRA_INPUT_LENGTH;
  // joint number
