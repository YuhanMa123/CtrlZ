 ...
 * 推理精度，用户可以通过这个参数来指定推理的精度，比如可以指定为float。
 * @tparam ActionPairs
 * 多个网络输出的ActionPair类型，用户可以通过这个参数来指定需要管理的网络输出的名称和类型。
 */
...
class ActionManagementWorker : public AbstractWorker<SchedulerType> {
 ...
 public:
  ...
  using OutPutRemapFunction = std::function<void(
      typename SchedulerType::Ptr,
      const z::math::Vector<InferencePrecision, ActionElementSize__>)>;
 ...
 public:
   ...
   */
  ActionManagementWorker(SchedulerType::Ptr scheduler,
                         const nlohmann::json& worker_cfg)
    ...
    this->ActionRemapFunctions__.fill(
        std::bind(&ActionManagementWorker::DefaultActionRemapFunction, this,
                  this->Scheduler, std::placeholders::_2));
    InferencePrecision DefaultSwitchIntervalTime =
        worker_cfg["SwitchIntervalTime"].get<InferencePrecision>();
    this->DefaultNextActionCycleCnt__ =
        static_cast<decltype(this->DefaultNextActionCycleCnt__)>(
            DefaultSwitchIntervalTime / this->CycleTime__);

    std::cout << "DefaultSwitchIntervalTime=" << DefaultSwitchIntervalTime
              << std::endl;
  ...
  void SetActionRemapFunction(OutPutRemapFunction func) {
    ...
    static_assert(idx != sizeof...(ActionPairs),
                  "ActionPair not found in ActionManagementWorker");
  ...
  void SetActionRemapFunction(OutPutRemapFunction func) {
    ...
    static_assert(CT.dim == ActionElementSize__,
                  "ActionPair size not match, the size of ActionPair must be "
                  "the same as ActionElementSize__");
    static_assert(idx != sizeof...(ActionPairs),
                  "ActionPair not found in ActionManagementWorker");
    ActionRemapFunctions__[idx] = func;
  }
   ...
   *
   * @param ActionName
   * 网络输出的名称，用户可以通过这个名称来指定需要切换到的网络输出。
   * @param SwitchIntervalTime
   * 切换间隔时间，用户可以通过这个参数来指定切换的间隔时间，网络将会在用户指定的时间后切换到指定的网络输出。
   * @details
   * 如果SwitchIntervalTime为-1，则使用默认的切换间隔时间。（切换单位为秒）
   * @return true 切换成功
   * @return false 切换失败，网络输出的名称未找到。
   */
  bool SwitchTo(std::string ActionName,
                InferencePrecision SwitchIntervalTime = -1) {
    this->block__ = false;
    size_t idx = ActionPairs__.index(ActionName);
    if (idx == sizeof...(ActionPairs)) {
      std::cerr << "ActionManagementWorker: Action not found in "
                   "ActionManagementWorker"
                << std::endl;
      return false;
    }
   ...
   * @brief 切换到指定的网络输出，用户可以通过这个方法来切换到指定的网络输出。
   *
   * @tparam CTSPair ActionPair
   * 网络输出的名称类型对，用户可以通过这个名称来指定需要切换到的网络输出。
   * @tparam InferencePrecision SwitchIntervalTime
   * 切换间隔时间，用户可以通过这个参数来指定切换的间隔时间，网络将会在用户指定的时间后切换到指定的网络输出。
   * @details
   * 如果SwitchIntervalTime为-1，则使用默认的切换间隔时间。（切换单位为秒）
   * @return true 切换成功
   * @return false 切换失败，网络输出的名称未找到。
   */
  template <CTSPair ActionPair, InferencePrecision SwitchIntervalTime =
                                    static_cast<InferencePrecision>(-1)>
  bool SwitchTo() {
    this->block__ = false;
    static_assert(ActionPair.dim == ActionElementSize__,
                  "ActionPair size not match, the size of ActionPair must be "
                  "the same as ActionElementSize__");
  ...
  /**
   * @brief
   * 阻止输出，用户可以通过这个方法来阻止输出。输出被阻止后，网络输出将不会更新电机目标位置，
 ...
 private:
  void DefaultActionRemapFunction(
      SchedulerType::Ptr scheduler,
      const z::math::Vector<InferencePrecision, ActionElementSize__>& data) {
  ...
  bool switchTo(const size_t idx, InferencePrecision SwitchIntervalTime) {
    ...
    if (SwitchIntervalTime == -1) {
      ...
      this->NextActionCycleCnt__ =
          static_cast<size_t>(SwitchIntervalTime / this->CycleTime__);
    }
 ...
 private:
  std::array<OutPutRemapFunction, sizeof...(ActionPairs)>
      ActionRemapFunctions__;
  ...
  // static_assert(isAllSameType<ActionPairs...>(), "All ActionPairs must be the
  // same type."); static_assert(std::is_same_v<InferencePrecision, typename
  // ActionPairs::type>, "InferencePrecision must be the same as ActionPairs
  // type.");
  static_assert(sizeof...(ActionPairs) > 0,
                "ActionPairs must be at least one.");
};
};  // namespace z
