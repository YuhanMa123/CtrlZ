 ...
 * //explain this create a compile time string key-value pair with key
 * "InferenceTime" and value type float.
 * //this can be used in CTSMap and work as fundimation to get and set value in
 * DataCenter, Scheduler, and Workers classes.
 */
template <CTString CT, typename T, size_t Dim>
...
void PrintCTSPairInfo() {
  std::cout << "[\"" << CTS.str.value << "\"] = "
            << typeid(typename std::remove_pointer_t<decltype(CTS.type)>).name()
            << std::endl;
}
...
class CTSArray {
 public:
   ...
   */
  static constexpr size_t size() { return sizeof...(CTS); }

  /**
   * @brief get the index of a compile time string in the array during compile
   * time
   *
  ...
  static constexpr size_t index() {
    constexpr auto it = std::find(string_array_.begin(), string_array_.end(),
                                  std::string_view(CT.value));
    constexpr size_t idx = it != string_array_.end()
                               ? std::distance(string_array_.begin(), it)
                               : sizeof...(CTS);

    // constexpr auto error_message = concat("CTS not found in CTSArray: ",
    // CT.value);
 ...
 private:
  constexpr static std::array<std::string_view, sizeof...(CTS)> string_array_{
      CTS.value...};
};
...
class CTSMap {
 public:
  ...
  void get(T& value) {
    ...
    // constexpr auto error_message = concat("CTS not found in CTSMap: ",
    // CT.value);
    static_assert(idx != sizeof...(CTS), "CTS not found in CTSMap");
   ...
   */
  size_t index(const std::string_view& str) { return ctsArray.index(str); }
