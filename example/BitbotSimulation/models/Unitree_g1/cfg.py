# 下面这个建模这么详细的是beyondmimic很牛逼的做法，对每一个电机里面的东西计算叹为观止
...
# Articulation：关节系统，所以这东西是个关节系统的配置
UNITREE_G1_29DOF_CFG = ArticulationCfg(
    spawn=sim_utils.UrdfFileCfg(
        ...
        # 执行器属性，这段是在给“整机关节系统（Articulation）的根级物理属性”做配置，主要影响 PhysX 对这个机器人关节系统的碰撞与约束求解方式：
        articulation_props=sim_utils.ArticulationRootPropertiesCfg(
            enabled_self_collisions=True,  # 自碰撞
            # 每个仿真步里，约束求解器用于位置层面（关节约束/接触穿透修正等）的迭代次数
            solver_position_iteration_count=8,
            # 每个仿真步里，约束求解器用于速度层面（摩擦、反弹、速度约束等）的迭代次数
    ...
    actuators={
        "legs": ImplicitActuatorCfg(  # 隐式PD执行器，不直接控制电机力矩，主要是通过位置计算力矩
            joint_names_expr=[
                ".*_hip_yaw_joint",
                ".*_hip_roll_joint",
                ".*_hip_pitch_joint",
