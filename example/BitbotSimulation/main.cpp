...
int main(int argc, char const* argv[]) {
  ...
  kernel.RegisterEvent("joystick_x_change",
                       static_cast<bitbot::EventId>(Events::JoystickXChange),
                       &EventJoystickXChange);
  kernel.RegisterEvent("joystick_y_change",
                       static_cast<bitbot::EventId>(Events::JoystickYChange),
                       &EventJoystickYChange);
  kernel.RegisterEvent("joystick_yaw_change",
                       static_cast<bitbot::EventId>(Events::JoystickYawChange),
                       &EventJoystickYawChange);

  // 注册 State
  kernel.RegisterState(
      "waiting", static_cast<bitbot::StateId>(States::Waiting), &StateWaiting,
      {static_cast<bitbot::EventId>(Events::SystemTest), (Events::InitPose)});

  kernel.RegisterState("SystemTest",
                       static_cast<bitbot::StateId>(States::PF2SystemTest),
                       &StateSystemTest, {});

  kernel.RegisterState("init_pose",
                       static_cast<bitbot::StateId>(States::PF2InitPose),
                       &StateJointInitPose, {(Events::PolicyRun)});
