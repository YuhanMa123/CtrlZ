...
void ConfigFunc(const KernelBus& bus, UserData& d) {
  ...
  d.NetInferWorker =
      d.TaskScheduler->template CreateWorker<BeyondMimicUnitreeInferWorkerType>(
          cfg_workers["NN"], cfg_workers["MotorControl"], JOINT_ID_MAP);
  d.TaskScheduler->CreateTaskList(
      "InferTask", cfg_root["Scheduler"]["InferTask"]["PolicyFrequency"]);
  d.TaskScheduler->AddWorkers(
      "InferTask", {d.NetInferWorker, d.ActionManagementWorker, d.Logger});
...
std::optional<bitbot::StateId> EventInitPoseFunc(bitbot::EventValue value,
                                                 UserData& d) {
...
std::optional<bitbot::StateId> EventPolicyRunFunc(bitbot::EventValue value,
                                                  UserData& d) {
  if (value == static_cast<bitbot::EventValue>(bitbot::KeyboardEvent::Up)) {
    std::cout << "policy run\n";
...
std::optional<bitbot::StateId> EventDanceFunc(bitbot::EventValue value,
                                              UserData& user_data) {
  if (value == static_cast<bitbot::EventValue>(bitbot::KeyboardEvent::Up)) {
    std::cout << "dance\n";
